...
export default function MenuItem({label, Icon, className, textClassName, ArrowIcon, onClick}: Props) {
  return (
    <div
      className={`flex h-[50px] cursor-pointer items-center gap-1 rounded-medium bg-white px-2 shadow-default3 ${className}`}
    >
      <div>{Icon || <Maximize />}</div>
      <p className={`subtitle flex-1 ${textClassName}`}>{label}</p>
